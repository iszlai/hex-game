...
A three-line brief (§12.2) explains the mode before a run starts — nothing else in the game does, and
"no undo" is a rule a player would otherwise discover by pressing undo (C-38).
...
so a shared board is comparable between players. Briefed before it starts, the same way Endless is
(§12.2, C-38).
...
|---|---|---|
| Boot | Title over the backdrop, 3 s max, skippable by any input. The **only** splash — the engine's is off (C-31) | — |
| Main menu | Campaign (with % complete), Endless (with best), Daily (with streak + timer to reset), Settings, Quit | Campaign |
| Mode brief (modal) | Three lines on what Endless or the Daily *is*, that mode's own record, Play / Back. Shown before either starts (C-38) | Play |
...
> **C-28 is reversed by C-30 on 2026-07-31.** C-28 stopped drawing the path as a line while the
> level was played, on the argument that two filled cells side by side already say they are joined.
> That holds for a line, and §5.1 makes the path a **tree**: on one that has doubled back, two
> adjacent filled cells are frequently not joined at all. The line is drawn as it is built again,
> and the trace on the winning move is a reprise of it rather than the only time it exists.
...
|---|---|---|---|
| Display / titles | Belligerent Madness | 48 px | Hand-drawn; **static**, no `wght` axis — C-30 |
| Headings | Caveat Bold | 32 px | Handwritten — C-30 |
...
Every face must carry a real `wght` axis — Medium and Bold are a variation, not a second file — and
the numeral face must have tabular figures. **The display face is exempt from the axis** (C-30): it is
drawn at one size on one screen and has nothing to vary. It is also the one face not under the OFL,
and its licence carries conditions the OFL does not — see C-30. **The display and heading faces are characterful and
therefore carry few symbols**, so the families fall back to one another: a glyph missing from one is
drawn by another rather than by an empty box (C-30).
...
across a hex board rather than a flood of it, and by the coverage measure a *completed* level reaches
40% in none of chapter 1's twelve levels and one of chapter 2's — the layer would never arrive for the
first two hours of the game (C-41).
...
| C-3 | Renderer choice for the Linux/Deck export (Forward+ vs Mobile) | Measure both at M7 against the §20 budget; pick the faster, document the result. **Partly measured at M7** — `make measure METHOD=…`, harness in `tools/measure_renderer.gd`. Three findings, none of them a decision yet. (1) The project has been running **`gl_compatibility`** all along, set at M0 and never revisited, so the question was never between the two options C-3 names — the whole C-18 board was authored and verified under a third renderer. (2) All three render the board correctly: no shader in `src/view/shaders/` fails to compile under any of them, which was the export-time failure worth finding early. (3) Draw calls and video memory differ in a way that matters against §20 — gl_compatibility 39 calls / 22 MB, Forward+ 38 / 100 MB, Mobile **42** / 83 MB. §20 budgets ≤ 40 draw calls and ≤ 100 MB, so Mobile is already over the call budget and Forward+ is at the memory ceiling, on a board with no HUD art yet. Frame *cost* is still unmeasured for the two RenderingDevice backends: macOS/Metal ignores the vsync disable, so both come back pinned to the panel's 8.33 ms. The decision needs a run on Deck hardware or on a Linux box where vsync can be turned off, and it belongs before the export presets are fixed at M11 |
...
| C-6 | Music production route (commission vs licensed library) | **Composed in the repository** as a placeholder that actually satisfies §15.1 (C-40): `tools/make_music.gd` holds the score and renders each bed's three stems in one pass. A commission or a licence is still the ceiling, and when one arrives it must ship as **one session exported three times** — three independent renders are three performances and will not layer. Either way the written commercial licence is retained in the repo |
...
### Resolved during implementation (M0–M7)
...
| C-21 | C-18 fixes the camera as orthographic with a *fixed pitch*, yawing between the six 60° stops, *tweened* — but gives no numbers: no pitch angle, no tween duration, and no rule for **when** §4.4's fit is evaluated as the board turns | **Elevation 55°** above the board plane. 90° is the 2D grey-box seen from straight above and 0° is edge-on; at 55° the plane's depth foreshortens to sin 55° = 0.82, which still reads unambiguously as a hexagon, while cos 55° = 0.57 of a prism's height projects as its visible side, so tile thickness and drop shadows have something to occupy. **Yaw tween 260 ms, `CUBIC` / `EASE_IN_OUT`**, ×0.4 under Reduce Motion like every other duration (§14.5) — between §14.1's 220 ms placement pop and its 320 ms screen transition, because the board never leaves the screen but the player must see which way it went. **The fit is swept across the rotation, not evaluated at the stops.** The six stops project to identical bounds (a radius-R hexagon of cells is invariant under a 60° turn, so the board never resizes when the player rotates), but the yaws in between bulge — the hexagon presents its corners where it presented its flats — so fitting the stops alone lets the board clip the play area for a tenth of a second on every rotation. `BoardCamera.fit_projected` takes the worst case over one 60° sector sampled per degree, giving `s` = **82 / 59 / 46** for radius 2 / 3 / 4 in §4.4's 880×688 box, against 74 / 53 / 42 head-on: the foreshortening buys size. One world unit is one pixel (orthographic height = the play box's height), so §4.4's 48 px margin stays literally 48 px. `tests/unit/test_board_camera.gd` pins all three, and pins "clockwise" against what a player sees rather than against which way the camera orbits |
| C-22 | §21 requires every visible state to be distinguishable without colour, and §13.3's flat board did it with glyphs, strokes and hatching. C-18 replaces that board with lit 3D prisms, where a stroke is not a thing you have and lighting can wash a fill out — so the colour-independent channel has to be re-chosen | **Height carries the kind.** A tile's top sits at a fixed fraction of the cell circumradius: empty **0.16**, joined **0.28**, wall **0.52** (`BoardTiles`). An empty cell is a thin plate, a joined cell stands proud of the board, and a wall is a block — an ordering that survives a greyscale palette, a monochrome screenshot and any lighting, because it is geometry rather than shading. Glyphs stay as the second channel for the modifiers (goal, portal, gate, wild), which are marks on a tile rather than kinds of tile. Two consequences: the tallest of these is what §4.4's projected fit has to reserve room for (`BoardView3D.TILE_TOP_RATIO`, and C-21's `s` = 82 / 59 / 46 already includes it), and the heights are ratios rather than pixels so the reading holds at every board radius |
...
| C-25 | §9 says the level select "uses the same board renderer — no separate UI system", and neither renderer can be taken at its word: [BoardView] and [BoardView3D] both bind a `GameState`, in which a cell is empty, walled, on the path or a goal. None of those is what a map cell is. Rendering "level 7, two stars, locked" through a game state means encoding progression as walls and path — which works for exactly as long as nobody changes the path gradient or C-22's tile heights underneath it | A dedicated `src/ui/hex_flower.gd`, which reuses the part of the renderer that actually makes this a hex map: `HexLayout`'s §4.3 conversion, its corner geometry, its `from_pixel` rounding for hit-testing (B7) and a fit rule written against the flower's own span, plus every colour from `Palette`. There is still exactly one hexagon formula in the codebase, which is the thing §9's sentence protects. The three map states get three silhouettes rather than three colours (§21), and the silhouette is the tile's **height**: a locked level is a wall, a completed one is path and is filled with the path colour as §9 asks, an open one is the empty cell a tile can be placed on. Navigation is `InputRouter`'s unchanged — the same ±75° cone over the same lattice, so left means the same thing on the map as on the board **Amended 2026-07-31**, once C-26's illustrated direction had landed on the board and not on the map: what a map cell now borrows from a board tile is its **material and its height**, which is the half of §9's sentence that was worth keeping. Each cell is drawn with `assets/art/tile_face.png` — the same 2×2 atlas of drawn stone faces, sampled by the same convention `hex_prism.gdshader` uses, so the two surfaces are one surface — and stands on a body in `board_tile_side`, at the height C-22 gives the board tile that map state *is* — asked of `BoardTiles.top_ratio` rather than copied out of it, so a retune moves the map with the board. The mapping is the one the player has already learned: locked is a **wall**, the board's word for not through here and the tallest thing on it; completed is **path**, standing proud the way a joined cell does; open is an **empty cell**, the plate a tile can be placed on, which is what an open level is. An earlier pass put the three in progress order instead — locked lowest, finished tallest — which was tidy and said nothing, because progress is already what the colour and the pips are for. Height **replaces** the 45° hatch a locked level used to carry rather than joining it. Five ruled strokes over a drawn stone face read as damage to the drawing, which is what a hatch stops being the moment the surface under it has a texture of its own — and the reason the board needs one does not apply here: `wall.fill` and `cell.empty.fill` sit within 1% of each other in luminance, so on the board the pattern is the only cue left once colour is gone, whereas on the map the two are different heights and a height is a shape. The pip row, present on an open level and absent on a locked one, says it a second time. Two things came with it. The flower sits on an opaque plate of `bg.panel`, because the board has a board plane and the map had an illustration: every gap between tiles was a window onto whatever the backdrop was doing there, and on chapter 1 that is the sun going down. And every level is numbered, locked ones included — eleven identical hatched blanks and one number is not a map. The drawn face is a value map that the board's shader doubles before multiplying; a canvas polygon has no shader to do that in, so the gain rides in the vertex colour, at 1.65 rather than 2.0 because a single multiply cannot also apply `FACE_STRENGTH`'s lerp and a path-coloured tile clips without it |
...
| C-28 | §13.1 makes the path "a single continuous line of light that grows and pulses", and the board draws a connector between every joined pair for the whole of a level. It is saying something the board already says: a placed cell is *filled* with the path colour, and two filled cells sitting next to each other are already telling the player they are joined. The line said it a second time, over the top of the tiles, on every move | **The lattice connectors are not drawn during play.** The route is still built, ordered and ready — it is simply not shown until the level is won, and then it draws itself from the start out to the goal as the thing the player gets for finishing. Decided 2026-07-31. Three consequences worth stating. (1) **Portal tethers stay.** A portal joins two cells that are *not* neighbours, so adjacency implies nothing and with no line at all nothing on the board says the path jumped; they are the one connection the cells cannot speak for. (2) §14.1's `connector_draw` row is unchanged and still wired — what moved is *when* it runs, which is once the trace has revealed the ribbon. (3) The reveal is by **count** over bars sorted from the start outward, so the same buffer and the same single draw call serve both states, and a branching path still reads as one line leaving the start rather than as bars appearing in whatever order the player happened to place them |
| C-30 | C-28 (above) stopped drawing the lattice connectors during play, so the path was read off the filled cells alone. Two things were wrong with the reasoning. The claim "two filled cells sitting next to each other are already telling the player they are joined" is true of a *line*, and §5.1 makes `path` a **tree** — on a route that has doubled back, two adjacent filled cells are frequently not joined at all, and nothing on the board said which. The codebase had already accepted this exact argument one layer earlier: `board_seams.gd` lights the entry edge of a candidate because "on a path that has doubled back the cell a given candidate connects from is genuinely ambiguous", and committing the move does not resolve the ambiguity, it freezes it. The second problem is what it cost: §2.1's pitch, §13.1's "single continuous line of light" and the game's own name all describe a thing the player never saw while playing | **C-28 is reversed.** The connectors draw as they are made, `_traced` starts at one, and §14.1's `connector_draw` beat runs on every placement rather than only after the level is over. Decided 2026-07-31. C-28's payoff is kept and re-framed: `play_trace()` still runs on the winning move, now as a **reprise** — the same line redrawn from the start outward, in the order it means rather than the order the player happened to build it in. Portal tethers are unaffected; they were always drawn and C-28's exception for them is simply no longer an exception |
| C-31 | The connectors C-30 brought back were **ruled**: dead-straight bars between two cell centres, one weight for their whole run. §13.1 (as C-26 rewrote it) makes the game "a hand-drawn scene the player is looking into, not a diagram they are reading", and a ruled line is the diagram. §13.1 also asks for "a single continuous line of light that grows **and pulses**", and the pulsing half had never been built — §14.1 tabulates a 300 ms flow pulse on *placement* and nothing standing | **The stroke is drawn rather than ruled, and it pulses along its own length.** Decided 2026-07-31. The bar mesh is sliced into `BAR_SEGMENTS` rings so a vertex shader has something to bend; each bar bows and varies in weight off three sine octaves seeded per instance, the third octave short and sharp enough to read as the tremor in a drawn stroke. The envelope is `sin(pi·t)`, **zero at both ends**, so however far a bar wanders in the middle it still meets its neighbours exactly at the tile centres they share — without that every joint in the route would show a step, and a line made of disconnected pieces is worse than a straight one. The seed is taken from where the bar *is* rather than from its index, so the ribbon does not redraw itself differently on every move and undo. The pulse rides a coordinate that spans the whole route (`INSTANCE_CUSTOM.g` = the departure depth, `.a` = one step), normalised by the **deepest** cell rather than the cell count, because §5.1 makes the path a tree and dividing by the count would stop the wave short of the far end. `PULSE_SECONDS` is a chosen number and says so, the way C-21's camera angles are: §14.1 never tabulated it, and §14.1's table is asserted row for row and may not grow a fifteenth entry. Zero stops it, which is what §14.5 asks of a loop. **Three refinements followed from looking at it.** (1) An **ink edge** across the ribbon's width, taken from `board_mark_outline` — the token the tiles and the marks already outline with, because one outline colour is what makes the board look drawn by one hand. Across the width only, never along the length: the caps at each end of a bar butt against the next bar's, and inking those draws a dark rung across the stroke at every joint. `LINK_WIDTH` widened 0.14 → 0.18 to keep the *lit* core as thick as it was before it had an edge. (2) The pulse **crackles** rather than sweeps — two octaves of cheap value noise running against each other break the travelling head into filaments, with a narrow smoothstep window so there is dark between them, plus a low-gain ember term so the stroke is never perfectly still between pulses. `LINK_GLOW` dropped 0.5 → 0.34 to give the filaments headroom; at the old constant emission they read as a slightly brighter smooth line. (3) A **bloom**, because light does not stop at the edge of the thing emitting it and the stroke was lit up to its ink edge and dark one pixel outside it. `Environment.glow_*` on the board viewport, thresholded at 0.92 — well above the tiles, which are lit rather than emissive, so only the path blooms and §13.7's painted backdrop is untouched. **This adds a post-process pass §20 has not measured**, and it should be measured with C-3's renderer decision rather than after it |
| C-32 | §4.4 sizes a board by radius and the generator only ever built hexagons, so all sixty campaign levels were one silhouette at one of three sizes. That is the whole of the game's board variety, and it is topological as well as visual: a hexagon has no corridors, no holes and no pinch points, so the only lever for making a route interesting is where the walls go. Growing the board is not the alternative — the solver's path mask is 64-bit, so 61 cells is the ceiling (C-19), and radius 4 is already too big for the difficulty tools to search | **Six board shapes**, decided 2026-08-01: hexagon, triangle, ring, corridor, hourglass, star. Each has a property a hexagon does not — corners a route can be trapped in, a hole that cannot be cut across, a width too narrow to wander in, a waist everything must pass, arms one cell wide. `Hex.shape(kind, size, arg)` builds them; the level schema gains `shape` and `shape_arg`, absent from every pre-C-32 file and defaulting to the hexagon they all are. Three consequences. (1) `Board.radius` is **derived from the cells**, because a shape's size parameter is not its radius — a triangle of side 6 is not a radius-6 board — while the file stores the size the shape was *asked* for, or reading it back would rebuild a different level. (2) Shapes are **centred** over their bounding region rather than on their nearest member cell: a ring's middle is precisely the hole, and centring on a cell put the hole off-origin and cost the ring the only property it exists for. (3) `Board.validate` checks the **cell count**, not `radius <= 4`, which was only ever a proxy for the mask; a triangle of side 7 reaches bounding radius 5 at 36 cells and is well inside it |
| C-33 | §9 orders the campaign by `par`, and `par` measures **length**. Three independent measurements say that is the wrong key: `tools/count_routes.gd` found chapter 4 to be the *widest* chapter in the game, `tools/measure_difficulty.gd` found it the most forgiving, and no cheap number predicts either (the best correlation against route count is `forgiving` at +0.44). The shipped sixty had **no difficulty curve in either dial** — chapter 4 level 6 had sixty perfect routes and sat between levels with two and one | **The campaign is authored against a written-down curve**, decided 2026-08-01. `tools/difficulty_curve.gd` holds it; `tools/author_levels.gd` generates *and measures* every candidate and keeps the best fit rather than the first acceptable one; `tools/level_metrics.gd` is the one implementation of both dials, so a report and the sweep can never disagree. Two dials, because they are nearly independent: **routes** (how many ways there are to finish in the ideal) and **forgiveness** (what fraction of wrong turns are recoverable). The shape is a **sawtooth**: levels 1–10 descend, 11 steps back, 12 drops — a chapter that only descends has no moment in it, because its hardest level is wherever the line happened to stop. Between chapters the *endings* escalate; the openings cannot, because §8.4 gives chapter 1 radius-2 tutorial boards that are structurally narrow whatever a curve asks for. A candidate the metrics cannot score inside their budget is **rejected**, which is also why C-32 reaches for shape rather than size: a board big enough to defeat the measurement is one the authoring step cannot rank. Each level records what it was authored to be, so CI checks the curve without re-measuring sixty boards |
| C-34 | `LevelRepository.id_for(chapter, index)` builds `"c3_l07"`, and `SaveService` keyed every star, best score and hint flag on it. That id names a **slot**, not a level: moving a level to another slot leaves its stars behind for whatever takes its place, so a player opens a board they have never seen already three-starred while the one they earned it on reads blank. Harmless while the campaign is never reordered, and the map editor of `docs/MAP-EDITOR.md` exists to reorder it | **Every level file carries a `uid`**, minted once and never reused, and progress keys on it. Decided 2026-08-01. Position becomes presentation: a level carries its stars wherever it moves, and *inserting* a level mid-campaign stops swapping everyone's progress down the chain. The uid is **random rather than a content hash**, which is the tidier-looking option and is wrong for the case it exists to serve — the editor is for tweaking a board, and a hash would give every tweak a new name and drop the stars earned on it. `Level.progress_key()` falls back to the slot id for a level built in code rather than loaded, which is every endless stage, every daily and every fixture; without it they would all share one row keyed on the empty string. The authoring sweep mints a **fresh** uid, because it produces a different level and inheriting the slot's old name is the same bug arriving from the other direction; the editor keeps the uid, because there it is the same level being edited |
| C-35 | A finished board was taken away from the player. In the campaign the results card arrived on a fixed 1600 ms tick and replaced the level screen; in endless, §7.2's "reaching a goal *is* the next stage" was taken literally and the board was rebuilt on the same frame the goal fell — so §14.2's flourish, burst and ripple and C-30's route trace all played over a board that had already gone. That is the one moment a player has any reason to look at the line they drew, which is the whole reason C-30 draws it and C-31 lights it | **A finished board is held until the player moves on, or three seconds.** Decided 2026-08-01. `GameDirector._stage_pause()` serves both modes — the next stage follows it in a run, the results card follows it in the campaign — after §14.2's beats and the trace, which are the part the player was never asked about. **Both** conditions, not either: a fixed wait makes someone who is ready sit still, and waiting only for input strands someone who put the Deck down mid-level. Any confirm, tap or menu accept ends it, through a named `EventBus.advance_requested` rather than a placement the won board would refuse, and the banner says so. One consequence worth stating: `endless_placements()` had to stop adding the live state's count on a *won* stage, because `advance()` has already banked it and the board now stays on screen long enough for the double to be visible; a *dead* stage was never banked and still adds |
| C-29 | C-23 draws §6's four modifier marks as procedural silhouettes, so they can be tinted through a palette token and stay readable once colour is gone. C-26's illustrated direction wants painted marks — a honey jar, a padlock — and painted art carries its own colour, which is exactly the property a tint cannot survive. Art that could not be tinted would look identical in all four of §21's alternates, undoing the thing those palettes exist to do | **Two complete presentations, chosen by the palette** (decided 2026-07-31). A palette declares itself `assistive`; §21's four alternates do, `cairn_warm` and `neon_dark` do not. An assistive palette gets C-23's silhouettes unchanged — nothing about them is weakened, and this is not a fallback in the sense of being worse. The other two may use an illustrated atlas (`assets/art/marks.png`, four cells in `Mark` order), drawn instead of the silhouette rather than over it, untinted because it is finished art. The silhouette remains the **floor**: no file, or an assistive palette, and the board draws what it always drew. This is the same arrangement §11.4 has for controller glyphs, where the text label is the floor under the icon, and for the same reason — an improvement that can go missing must never be the only thing holding a requirement up. What is deliberately *not* done is a hybrid: a colour layer over a tintable silhouette would make every mark a compromise in both directions |
...
| C-40 | §15.1's adaptive music needs each bed as three stems that play *together*, and the beds had been cut from two finished recordings. There was no session to go back to, so the layer could not be made: a generator produces another variation, never the same take with the pads muted, and two renders do not line up — they phase. The half of §15.1 that makes the music adaptive had therefore been unbuilt since M7, and the asset brief was asking for something no supplier could deliver | **The music is composed in the repository**, decided 2026-08-01. `tools/make_music.gd` holds a score — chords, a bar grid, one entry per part — and renders three stems in one pass, so muting a part is exactly what it sounds like and the files are aligned by construction. Consequences: the loop is seamless because the ring-out past the end is *folded back over the beginning* rather than cross-faded, so nothing ducks at the join; the three stems are normalised by **one** factor measured on `base + layer`, because per-stem normalisation would rewrite the arrangement and per-track normalisation left six beds 1.5 LU apart; and the render is deterministic off the track's name, so the output is committed and diffed rather than re-listened to. It is placeholder music a composer would beat, and it is the first version of it that §15.1's behaviour can actually be built on — which is now built: `AudioDirector` runs three players per deck and rides the layer on board fill |
| C-41 | §15.1 fades the second stem in "as board fill % rises above 40%". Measured over the shipped sixty, a *finished* level reaches 40% board fill in **0 of 12** levels in chapter 1, 1 of 12 in chapter 2 and 4 of 12 in chapter 4: a path is a line across a hex board, not a flood of it, so the fraction of cells a solution occupies is a fact about the board's shape rather than about the player's progress. Implemented literally, the adaptive half of §15.1 would never be heard for the first two hours | **The thresholds stay; the measurement changes**, decided 2026-08-01. The layer rides `placements / par` — how far into the level the run is — which reaches 1.0 on every board by construction, crosses 40% where the spec meant it to (a level that is getting somewhere), and falls back on an undo, which is the one thing the coverage number did get right. A level with no par falls back to coverage, because a constant would be worse than an imperfect measure. `AudioDirector` is told an *intensity* rather than a fill, so what it counts stays [GameDirector]'s business (§16.2) |
| C-42 | The composed beds of C-40 were **pads** — sustained chords held for a bar — and read as an organ. The cause is structural rather than a matter of voicing: a chord that starts and does not stop is an organ, and triads without sevenths are hymn harmony | **The beds are lo-fi jazz**, decided 2026-08-01: a Rhodes comping sevenths and ninths over an upright bass, tape hiss and crackle underneath, everything on a swung eighth grid. Every voice now *decays*, so the harmony is re-struck rather than held and the gap between strikes is where the room lives; the chords are voiced rootless, because the bass has the root. §15.1's "no percussion in campaign" holds — there are no drums, the swing is carried by the comping and the bass, and the vinyl bed is texture that cannot be counted. One consequence in the numbers: a struck piano's peaks are several times its average, so the peak ceiling was reached nearly 2 LU before §15.3's −16 LUFS and the bed came out quiet. A `tanh` saturation per stem — which is what a tape machine does about exactly that, and the sound the style is named for — brings it back to −16.6 |
