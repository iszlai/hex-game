...
> **Amended by C-28 on 2026-07-31** on one point: the path is not drawn as a line
> *while the level is played*. A placed cell is filled with the path colour, and two
> filled cells side by side already say they are joined — so the line is kept for
> the moment the level is won, where it runs start to goal once. Portal tethers are
> the exception and stay, because a portal joins two cells that are not neighbours
> and nothing else on the board would say the path jumped.
