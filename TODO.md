...
| M10 | Accessibility & i18n | 🟨 palettes + i18n in | text scale audit, Reduce Motion sweep, greyscale playthrough |
...
left in M7 behind it: the last two beats of §14.2's goal sequence.
...
- [x] **C-29** — §6's marks may be illustrated, chosen by the palette. A palette now declares itself
      ...
      atlas is **painted art** now — a cairn, portal rings, a padlock and a star, cut from a
      generated sheet by `make marks-cut`. `make marks` still renders the drawn-in-code stand-in if
      the atlas is ever lost. **One known blemish**: the portal was drawn with a semi-transparent
      centre over a checkerboard preview, and a blend of paint and checker is not something a
      threshold can undo, so a faint pattern survives inside its innermost ring. Invisible at tile
      size; fixed properly by a sheet whose portal centre is opaque
...
- [x] Every §14.1 timing, exactly as tabulated — the table is pinned **and every row of it now
      ...
      **Wired so far:** candidate breathing and the goal pulse, both in the shader off `TIME` so
      ...
      still one tween and one number. and the queue advance, the auto-discard arc and the results
      stars — the three that were waiting for §12.3's real layout, all three now wired *and* watched
      running (`test_motion.gd`, `test_results.gd`). and the illegal shake, whose red flash survives Reduce Motion even
      ...
      black where nothing can be seen to pop. **The three that were outstanding are done**: the two
      rail beats had been wired and tested when §12.3's layout landed, and the results stars were
      wired but had never been watched run — code existing is not the criterion, so they now have a
      test. §14.1's stagger is asserted against the table rather than raced on screen: a headless
      frame is longer than 140 ms and `BACK`/`EASE_OUT` overshoots past its target, so a mid-flight
      sample can read a settled star as moving and a moving one as settled
- [x] Goal-reached sequence §14.2; camera limited to §14.3 (2 px shake, once per completion) —
      **all six beats**, off `Motion.GOAL_SEQUENCE` so the sequence is diffable against §14.2 line by
      line and the intervals are differences between table entries rather than durations typed into a
      script. The board owns five; the Results card at t=700 is [GameDirector]'s, because a screen is
      not the board's to raise. §14.3's 2 px shake is wired to completion with its whole budget
      enforced in code — 2 px, 120 ms, and "once per level completion, nowhere else" as a flag only a
      new level resets. The last one built was **t=340's settle**, and it was easy to miss because
      nothing looked absent: entering a goal makes it a path cell as far as the rules go, and the
      board followed at once, so the flourish, the burst and the ripple all played over a tile that
      had already stopped looking like a goal. It is held at the goal colour now and crossed over on
      the beat. §14.2 fixes that moment and not its duration, so `GOAL_SETTLE_MS` is a chosen number
      and says so
...
- [x] Buses and sliders, −16 LUFS / −1 dBTP, `place.*` voice cap 4 (§15.3) — Master → Music / SFX /
      UI, each effect naming its own bus so the two sliders move different sounds, and `place.*`
      capped at four voices with the oldest stolen. **The loudness targets are measured now**, which
      needed a real mix to measure: every bed's `base + layer` sits at −16 LUFS with peaks under
      −2 dBFS, and the renderer normalises to that rather than to a peak — six beds normalised to
      their own peaks were up to 1.5 LU apart, which reads as the volume moving when the chapter does
- [x] §15.1's adaptive music, both halves (C-40) — the stems and the behaviour. `tools/make_music.gd`
      **composes** the six beds and renders each as three stems in one pass over one score, so they
      are aligned by construction; a generator cannot do this, and neither can two renders of the same
      piece — they phase. The loop is seamless because the ring-out past the end is folded back over
      the beginning rather than cross-faded, so nothing ducks at the join (seam jump measured at 181
      against 981 for the loudest sample-to-sample step inside the file). `AudioDirector` runs three
      players per deck and rides the layer up above 40% board fill and down below 30% — §15.1's two
      ...
      **What it rides had to change (C-41).** §15.1 says "board fill %", and measured over the shipped
      ...
      §15.1 would never be heard for the first two hours of the game. The thresholds are the spec's;
      the number measured against them is `placements / par`
...
- [x] String extraction, and **the §22 literal check in `ci_gate.sh` is live** — it was scaffolded and
      skipping because the file it looked for did not exist (C-39). Every player-visible string in
      `src/` is now a key in `assets/i18n/strings.csv`: 218 rows, one column per language. The tables
      hold keys rather than sentences — the legend's rows and cues, the settings' labels and value
      names, the tutorial's beats, the 29 bindable actions — so a translator edits one file and
      touches no code. `tests/unit/test_localization.gd` reads the scripts back and fails on a key
      with no row, a row nothing uses, a language column that is blank, and a translation whose
      `{placeholders}` do not match its English
- [x] **Hungarian**, in full, beside the English — Settings → Language, first row of the first tab,
      because a player who has landed in a language they cannot read has one thing to look for. Applied
      by `SettingsService` before the first screen exists, so nothing flashes English on the way in;
      changing it rewrites the screen the player is standing in rather than rebuilding it
      (`EventBus.language_changed`). `HEXFLOW_LANG=hu make shot …` photographs any screen in it
- [ ] Pseudo-locale for clipping tests (§22). Hungarian runs 10–30% longer than the English and is a
      live check in the meantime — the mode brief's daily line was rewritten once already because the
      Hungarian wrapped
...
- [x] **Trace mode** (§4.1.1, §4.4.1) — a mode switch above the brushes; in trace mode a left click
      lays the next tile of the sequence and a right click takes it back. Added because **Fill does
      not always fill**: each of its ten seeds is a random walk that can paint itself into a corner,
      and on a tight board all ten do, leaving "no deal made this board solvable" about a board the
...
- [x] `assets/i18n/` holds `strings.csv` — every string in the game, in English and Hungarian. The
      `.translation` files beside it are generated by the import and gitignored, so a fresh clone
      rebuilds them; the CSV is the source. The rest of the tree is populated:
      `fonts/` (3 vendored), `sfx/` (16 synthesised), `music/` (6 beds × 3 stems, composed),
